...
def test_report_request_with_fresh_prompts_generates_immediately(
    client,
    test_engine,
    create_verified_user,
    simulate_webhook,
):
    """Test report request when all prompts are already fresh.

    When all prompts have fresh data (<24h old), the request should
    move directly to READY without BrightData trigger.
    """
    session_maker = _make_session_maker(test_engine)
    ...
    user_email = f"fresh-request-{uuid.uuid4()}@example.com"
    auth_headers = create_verified_user(user_email, "testpassword123", "Test User")
    ...
    group_id = _create_group_with_prompts(client, auth_headers, "FreshGroup", prompt_ids)
    ...
    webhook_items = [
        {"prompt": prompts_dict[p1["id"]], "answer_text": "Fresh answer P1", "citations": []},
        {"prompt": prompts_dict[p2["id"]], "answer_text": "Fresh answer P2", "citations": []},
    ]
    simulate_webhook(batch_id, webhook_items)

    # === STEP 3: Create report request (all prompts fresh) ===
    request_resp = client.post(
        f"/reports/api/v1/groups/{group_id}/request",
        json={"assistant_id": 1},
        headers=auth_headers,
    ...
    # All prompts were fresh, so should be READY immediately
    # (no BrightData batches to wait for)
    assert request_data["total_prompts"] == 2
    assert request_data["prompts_fresh_at_request"] == 2
    assert request_data["prompts_requested"] == 0

    # Request should already be READY since no batches to wait for
    request_obj = run_async(_get_report_request(session_maker, request_id))
    assert request_obj.status == ReportRequestStatus.READY
...
def test_report_request_duplicate_prevented(
    client,
    test_engine,
    create_verified_user,
):
    """Test that creating duplicate requests is prevented."""
    session_maker = _make_session_maker(test_engine)
    ...
    # === STEP 1: Create user and group ===
    user_email = f"dup-request-{uuid.uuid4()}@example.com"
    auth_headers = create_verified_user(user_email, "testpassword123", "Test User")

    prompts = _get_prompts_for_topic(client, auth_headers)
    p1 = prompts[0]
    group_id = _create_group_with_prompts(client, auth_headers, "DupGroup", [p1["id"]])
    ...
    # === STEP 3: Try to create second request - should fail ===
    dup_resp = client.post(
        f"/reports/api/v1/groups/{group_id}/request",
        json={"assistant_id": 1},
        headers=auth_headers,
    )
    assert dup_resp.status_code == 409  # Conflict
    assert "already pending" in dup_resp.json()["detail"]["message"]
