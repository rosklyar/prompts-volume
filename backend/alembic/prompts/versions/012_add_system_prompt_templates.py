...
_GENERIC_TEMPLATE = """\
You are an expert in creating search prompts for AI assistants in the {domain_name} domain.

CONTEXT:
- Business domain: {domain_name} ({domain_context})
- Keywords: {keywords}
- Language: {language}
- Generate exactly 1 prompt per keyword
...
def _generic(domain_context: str) -> str:
    """Build a generic template with domain_context baked in as literal text."""
    return _GENERIC_TEMPLATE.replace("{domain_context}", domain_context)
...
_DOMAINS: list[tuple[str, str, str]] = [
    ...
    ("crypto", "Cryptocurrency and blockchain", _generic("cryptocurrency, blockchain, digital assets, decentralized finance")),
    ("real-estate", "Property and real estate", _generic("property, real estate, housing, rentals, property investment")),
    ("entertainment", "Media and entertainment", _generic("media, streaming, gaming, content, leisure activities")),
    ("general", "Products, services, and solutions across various industries", _generic("products, services, and solutions across various industries")),
]
...
def downgrade() -> None:
    ...
    # Delete subcategory rows and general
    op.execute(
        sa.text("DELETE FROM business_domains WHERE name LIKE '%|%'")
    )
    op.execute(
        sa.text("DELETE FROM business_domains WHERE name = 'general'")
    )
