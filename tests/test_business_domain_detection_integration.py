...
@pytest.mark.asyncio
async def test_detect_ecommerce():
    ...
    service = BusinessDomainDetectionService(api_key=api_key, model="gpt-4o-mini")
    languages = ["English", "Ukrainian", "Russian"]
    business_domain, brand_variations = await service.detect("rozetka.com.ua", languages)

    assert business_domain == BusinessDomain.E_COMMERCE
